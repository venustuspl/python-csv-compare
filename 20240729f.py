...
try:
    ...
    file1 = pd.read_csv('file1.csv', encoding='latin-1', delimiter=';', header=None, dtype=str)

    # Filter file1 to only include rows where column 2 is in car_brands
    table1 = file1[(file1.iloc[:, 2].isin(car_brands))][[2, 3, 4, 7]]

    print("Table 1:")
    print(table1)

    # Read file2.csv with encoding latin-1, delimiter ';', and no header
    file2 = pd.read_csv('file2.csv', encoding='latin-1', delimiter=';', header=None, dtype=str)

    # Filter file2 to only include rows where column 0 is in car_brands
    table2 = file2[(file2.iloc[:, 0].isin(car_brands))][[0, 1, 2, 3]]
    ...
    # Export tableDiff to a CSV file with the current date and time
    current_datetime = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    tableDiff.to_csv(f"tableDiff_{current_datetime}.csv", index=False, header=False)
